...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		List<Integer> result = new ArrayList<Integer>();
			for(int i = 0; i < numVertices; i++) {
				int total = this.getNeighbors(i).size() + this.getInNeighbors(i).size();
				result.add(total);
			}
	...
	public static void main (String[] args) {
		...
		List<Integer> temp = graphFromFile.getDistance2(1);
		System.out.println(temp);
		
		GraphAdjMatrix mat = new GraphAdjMatrix();
		GraphLoader.loadRoadMap("data/testdata/simpletest.map", mat);
		List<Integer> temp2 = mat.getDistance2(1);
